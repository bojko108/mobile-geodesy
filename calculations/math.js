 ...
 * @param {!Array.<Number>} start - lon, lat coordinates
 * @param {!Array.<Number>} end - lon, lat coordinates
 ...
 *
 * @param {!Array.<Number>} start - first point coordinates in decimal degrees - lon, lat
 * @param {!Array.<Number>} end - end point coordinates in decimal degrees - lon, lat
 * @return {Object.<String,*>} result
 * @param {Number} result.distance
 * @param {Number} result.initialBearing
 * @param {Number} result.finalBearing
 * @param {Number} result.iterations
 */
export const distanceBearingOnEllipsoid = (start, end) => {
  ...
  do {
    ...
    sinSqsigma =
      cosU2 * sinLamb * (cosU2 * sinLamb) +
      (cosU1 * sinU2 - sinU1 * cosU2 * cosLamb) *
        (cosU1 * sinU2 - sinU1 * cosU2 * cosLamb);
    ...
    cos2sigmaM =
      cosSqalpha != 0 ? cossigma - (2 * sinU1 * sinU2) / cosSqalpha : 0; // equatorial line: cosSqalpha=0 (§6)
    ...
    var iterationCheck = antimeridian
  ...
  let alpha2 = Math.atan2(
    cosU1 * sinLamb,
    -sinU1 * cosU2 + cosU1 * sinU2 * cosLamb
  );
